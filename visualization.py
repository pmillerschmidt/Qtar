...
class TrainingVisualizer:
    ...
    def save_epoch(self, history, current_epoch):
        ...
        filepath = os.path.join(self.save_dir, f'training_progress_epoch_{current_epoch}_{timestamp}.png')
    ...
    def plot_training_history(self, history, show=True, save=True):
        ...
        if save:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            filepath = os.path.join(self.save_dir, f'training_progress_{timestamp}.png')
            plt.savefig(filepath)
            print(f"Training visualization saved to {filepath}")
