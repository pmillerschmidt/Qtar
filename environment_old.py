...
class QtarEnvironment:
    ...
    def _calculate_base_reward(self, note, rhythm, chord):
        ...
        reward += self._reward_harmony(chord, note_in_octave)
    ...
    def _reward_harmony(self, chord, note_in_octave):
        chord_tones = CHORD_TONES[chord]
        # if strong beat
        if self._is_strong_beat():
            if chord_tones[0] == note_in_octave:  # root
                return 6.0
            elif note_in_octave in chord_tones:
                return 4.0
            else:
                return -1.0
        # non-strong beat
        else:
            if note_in_octave in chord_tones:  # root
                return 4.0
            else:
                return 0.0
